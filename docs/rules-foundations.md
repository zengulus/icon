...
| K-P7 | Talent subfamily folds (remaining) | **Landed (2026-08-28, F8a):** action-cost-override fold via `CostModifierRule` in `kernels/cost-payment.ts` + `content/jobs/action-cost-override-recipes.ts` (3 pure mastery overrides promoted); **shove-modifier decomposed (F8b)**: 10 pure singletons reclassified with precise blockers (direction-override, conditional-distance-stun, new-shove-effect, foe-trigger-expansion); all resolver-level — no fold consumer. Kernel removed. Compound heave-ho:mastery reclassified to variable-cost only. **charge-state reclassified**: 6 delay-mechanic units removed; 3 resolver-implemented units (spinning-top:t2 → fly-grant, chaos-tarot:t2 → area-modifier, gigaton-whip:t2 → fly-grant+new-shove-effect) reclassified — Charge variants already wired via `context.triggers?.has('charge')` in resolvers. Only terraforming:t1 retains charge-state (Charge variant genuinely unimplemented). Census total 419. Remaining subfamilies: resource-management | cost-payment fold over mastery-equipped + round-gate | resource-management | Exact-ID slices, each with positive/negative/replay |
