...
| K-P7 | Talent subfamily folds (remaining) | **Landed (2026-08-28, F8a):** `kernels/action-type.ts` — generic action-type/action-cost fold with mastery rows (round-gated free action) and talent rows (terrain/condition-gated free action). Content rows in `content/jobs/action-type-recipes.ts`. 10 pure action-type-change units registered. Fold consulted at USE_ABILITY gate + EXECUTE_RULE cost-validation gate. Remaining subfamilies: resource-management, charge-state, shove-modifier | action-type fold over equipped/mastered + encounter-state predicate | resource-management, charge-state, shove-modifier | Exact-ID slices, each with positive/negative/replay |
