...
| K-P7 | Talent subfamily folds (remaining) | **Landed (2026-08-28, F8a/F8b):** action-cost-override fold via `CostModifierRule` in `kernels/cost-payment.ts` + `content/jobs/action-cost-override-recipes.ts` (3 pure mastery overrides promoted); shove-modifier fold via `ShoveModifierRule` in `kernels/shove-modifier.ts` + `content/jobs/shove-modifier-recipes.ts` (distance modifier registered; 5 direction modifiers and 4 new-effect units are resolver-level concerns, not fold-applicable). 3 granted free actions remain. Remaining subfamilies: resource-management, charge-state | cost-payment fold over mastery-equipped + round-gate; shove-modifier fold over source-actor talent gate | resource-management, charge-state | Exact-ID slices, each with positive/negative/replay |
