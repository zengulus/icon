...
| --- | --- | --- |
| **Power-die stance** — die size, start value, tick trigger, consume/empower, discard-at-0 | Soul Blade (`soul-blade:die`, d6@2), Wicked Sheath (`wicked-sheath:charged`, d4 + round-start tick + discard on hit), Gallows Humor (`gallows-humor:die`, d6@1 + tick on miss + empower), Umbral Echo (`umbral-echo:die`, d4@2 + turn-end refresh), Gran Reversa, Obsidian Flesh, Exorcism (d4 on a mark), Godly Smite mantra die | **partial → kernel landed** — `kernels/power-die.ts` (`readPowerDie`/`tickPowerDie`/`setPowerDie`) now the single source-ID-free tick/clamp/ownership rule; soul-blade/gallows-humor/umbral-echo/mantra ticks consolidated on it. `seer:gran-reversa:talent:1` (start d6@6) wired program-level and audit-complete. Remaining per-row: consum/reverse (soul-blade), wicked-sheath shove-per-charge, gallows-humor ally-defeat max, rampant-nail bloodied-count tick, mastery dice |
...
| **Gamble** — d6 (or die) roll, result branching, boundary pre-roll, stacked/gamble resources | Riposte, Spinning Top, Carnevale (turn-end), Monogatari (boundary), Party Favor (detonate), Death (space choice), Stack Dice (trait), Bend Fate (trait), Golden Mask (trophy, recorded d20/d10/d8/d6/d4) | **largely existing** — `context.dice.die()` resolves every gamble deterministically (dice owned by the command boundary: spinning-top, death, party-favor, carnevale, riposte, chaos-tarot all use it); `TurnDiceWindows` pre-rolls turn-boundary gambles. The three genuine `gamble-state` singletons (spinning-top:talent:1, pandaemonium:talent:1, bend-fate) each need an *additional* real capability (choice-input, direction-choice, 13-card deck) — there is no clean gamble-only unit to promote, so no further shared gamble effect is required |
| **Sacrifice** — HP-for-effect, flat vs percent, cost overrides | Dropkick (sacrifice 6), Takedown (sacrifice 4), Great Suplex (up to 6 + fly half), Wolfheart (25%), Inner Furnace (Infuse cost), Crimson King ranks (cost overrides) | **flat landed (F14)** — `primitives/cost-payment.ts` pays a fixed HP cost (unmitigable, floor 1, no when-damaged window); **percent (25% max) and split payments remain** (`sacrifice-percent`) |
| **Ability-use blessing/combo spend** — spend 1 token for a package, 3 for the bigger effect; spend combo to activate charge | Blessing of Rebirth / War (traits) ✅ wired via the F10 choice fold; Blessing of Faith, Songweave, Divine Grace, Felicity, Umbra talent 2 still need rows | **landed (F10)** — `kernels/ability-use-choices.ts` (source-ID-free) + `content/jobs/ability-use-choice-recipes.ts`; a permanent choice→cost→modifier seam, also reusable for combo spend and extra-gamble-dice spends |
...
| **Infuse / Aether cost kernel** — spend Aether for a variant, cost reductions | Wright class trait, Conqueror's Edge, Inner Furnace, relic invokes with Aether spends | **landed (F14)** — `kernels/cost-payment.ts` cost-modifier registry + `effectiveRuleCosts` (reduce an Aether/Infuse cost, replace a payment, alter a fixed sacrifice); per-ability spend limits remain |
...
1. ~~Resource-economy / spend primitives~~ — **landed (F14)**: the HP-payment
   seam (sacrifice), ability-use spend (blessing/combo), Infuse-cost, and the
   shared use ledger now live in `primitives/cost-payment.ts` +
   `kernels/cost-payment.ts` + `kernels/use-ledger.ts`;
