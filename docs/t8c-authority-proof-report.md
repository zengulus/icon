...
| U7 | From where is this spatial rule measured? | `primitives/anchor.ts`+`spatial-intent.ts` | candidate range-origin, choice position range | `RuleArea.origin`, teleport arbitrary-origin, creation spatial contract — Q: domain-specific placement/legality; disjoint from generic anchor resolution; guarded | aura/teleport/creation origins not unified onto `SpatialAnchor` | PARTIAL |
