...
| Jobs | Yes | Partial | All 16 Jobs and all 144 abilities have independently reviewed typed programs, resolvers, and source-page replay fixtures. Diaga, Bless, and Succor (Mendicant p.172) are also reviewed. All 65 Job traits are cataloged in the closed `JOB_TRAIT_RECIPES` inventory (27 wired with engine mechanics across the F6 homes — condition projections, combat-start grants/companion summons, lifecycle recipes incl. the round-start phase, typed resolvers, the shared attack-path modifier kernel covering Demon Edge / Hissatsu / Pulverize / Bull's Strength and Trigrammaton's exactly-range-3 row, Shieldmaster's aura + turn-end membership recipe, the F9 once-per-round reactive job-trait fold with its durable round ledger (Dash on the Rocks, p.230), and command/kernel hooks — and 38 documented table-facing rulings); see `docs/job-trait-template.md` and `rules-foundations.md` §7. All 288 talents are cataloged in the closed `TALENT_RECIPES` inventory (`content/jobs/talent-recipes.ts`): a wired tranche (30) executes through the shared `talentTriggerMutations` fold (`exceed` on attack roll 15+, `comeback` while bloodied — incl. Riposte t2, the `finishing-blow` trigger with per-row eligibility extensions — Party Favor t2, the `always` trigger for unconditional charge-scaled augmentations — Dropkick t2, the single-foe condition-grant family reading the ability's own mutations — Valiant t2, Provoke t1, Showdown t2, and the post-application `slay`/`collide` triggers decided by the same reactive dry run that derives the ability's own clauses — Umbra t1, Valiant t1, Dragon Dive t1), five program-level talents (Demon Cutter t2's pre-ability rush, charge: rush 3; Draken Cross t2's charged medium blasts; Pyre t1's comeback ally immunity; Divine Aegis t2's quarter-HP defiance; and Eye of the Storm t2's area-count piercing) are implemented in the ability programs and gated on the equipped choice through the projected `talents` surface, and the 245 documented rows carry their kernel need; the actor's durable `talents` map is validated by the room validator and protocol schema — see `rules-foundations.md` §8. The unfinished Job scope is the 38 documented traits, the 245 documented talents, masteries, Limit Breaks, summon action suites, and documented table-facing choices—not a source-only set of Job abilities. |
...
| Foes | Yes | Partial | Six source roles and 449 jobs, variants, uniques, elites, legends, components, and special entries with 1,365 abilities; standard profile construction works. 22 foe abilities across the Crusher (p.301), Warrior, Soldier, Brute (p.300), Pepperbox, Hunter (p.302), Cantrix (p.305), and Chaos-Wright (p.306) profiles are independently executable as declarative recipes, and 79 reviewed foe-trait keyword rows are fully executable (p.298 glossary + p.104 conditions/Defiance/Size/Armor/Speed) with 36 more partially projecting their wired keywords (see the foe recipe sections). Mobs, the remaining 1,247 unsupported foe abilities, and the remaining 590 traceable foe traits are not executable. |
...
Fidelity notes are preserved on the program: Carnevale's optional post-summon dash is left to the normal movement command and its turn-end detonation is taken deterministically; Party Favor's mine detonates through the `kernels/movement-triggers.ts` voluntary-MOVE fold whenever a standard MOVE enters its space (the gamble is pre-rolled at the command boundary); and Gallows Humor's empower tracks the die in `ruleState` so the resolver can read it through the generic VM actor view.
...
Fidelity notes are preserved on the recipes themselves: deterministic defaults take the first-listed branch of "either/or" and "may" choices (the alternative is table-facing), critical hits add one extra [D], bonus damage (p.102) rolls an extra die and keeps the higher result, collide detection mirrors the reducer's `shoveResolution`, and the `end-turn` mutation is what auto-ends the foe's turn through EXECUTE_RULE. Replay fixtures in `__tests__/foe.test.ts` pin the golden mutation sequences and verify `applyEvents` replays to the identical state. The seven reviewed profiles still carry table-facing `foe-trait` units where the source row is prose rather than a pure keyword list (for example, Hunter's Wayfinding); the reviewed foe-trait keyword projection is documented below.

## Foe-trait keyword projection (ICON p.298 glossary + p.104 positive effects)

`automation/content/foes/trait-recipes.ts` is a closed source-ID manifest, not a text parser. It maps 115 reviewed `special-traits`/`traits` rows whose extracted text is a pure keyword list (Sturdy, Defiance, Flying, Size 2, Armor 10, …), with each keyword's durable effect declared in `automation/kernels/foe-trait-recipes.ts`:

- **Consumed conditions** (91 projections): Sturdy, Flying, Phasing, Regeneration, Skirmisher, Dodge, Aetherwall, Slip, Rampart, and Immobile fold into `encounterConditionSet`, so the movement planner, damage kernel, and trigger paths all see the same set. This replaced the old 36-ID movement-only slice with the full p.104/p.298 condition family.
- **Durable stats** (37): `Size`, `Armor`, and `Speed` keywords override the role defaults once at profile construction; the durable actor fields are the record, so replay never re-derives them. The p.92 footprint half of Size stays pending.
- **Durable combat-start grants** (17): Defiance is granted once on `ENCOUNTER_STARTED` (never projected — a projection would resurrect it after the damage kernel consumed it). The damage kernel consumes it exactly once and sets turn-end `damage-immune` (p.104).
- **Role baselines** (2): Guard rows resolve to the heavy role's baseline (Guard = Rampart + armor 2, already consumed by the role machinery).
- **Explicitly pending** (2): Counter and Diaga rows project their wired keywords while auditing incomplete with the exact pending reason.

79 rows are fully executable (`automation: 'executable'`, audit-complete); the other 36 are partial rows whose wired keywords still project while their Counter/Diaga/Size-footprint clause stays source-visible. Nothing is inferred from prose at runtime: a row only projects when its source text parses to exactly the declared keyword list (including source artifacts such as `S ize 3`, lowercase `size 2`, or a trailing period on `Regeneration.`). Projections are passive — they never open an `EXECUTE_RULE` command.

`__tests__/foe-traits.test.ts` table-audits every registered row against its exact source keyword list, proves all unregistered keyword-list traits stay unprojected (closed negative), pins the durable stats and Defiance consumption with replay fixtures, and replays movement for real Hellion (Flying), Shadow (Phasing), and Smoke Demon (Phasing + Flying) profiles through the authoritative reducer.
...
| Party Favor — movement-entry detonation | p.151 | wired (mine entry; detonate sub-action remains manual) |
