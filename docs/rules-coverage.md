...
| Jobs | Yes | Partial | All 16 Jobs and all 144 abilities have independently reviewed typed programs, resolvers, and source-page replay fixtures. Diaga, Bless, and Succor (Mendicant p.172) are also reviewed. All 65 Job traits are cataloged in the closed `JOB_TRAIT_RECIPES` inventory (23 wired with engine mechanics across the F6 homes — condition projections, combat-start grants/companion summons, lifecycle recipes incl. the round-start phase, typed resolvers, the shared attack-path modifier kernel covering Demon Edge / Hissatsu / Pulverize / Bull's Strength, the F9 once-per-round reactive job-trait fold with its durable round ledger (Dash on the Rocks, p.230), and command/kernel hooks — and 42 documented table-facing rulings); see `docs/job-trait-template.md` and `rules-foundations.md` §7. All 288 talents are cataloged in the closed `TALENT_RECIPES` inventory (`content/jobs/talent-recipes.ts`): a wired tranche (29) executes through the shared `talentTriggerMutations` fold (`exceed` on attack roll 15+, `comeback` while bloodied — incl. Riposte t2, the `finishing-blow` trigger with per-row eligibility extensions — Party Favor t2, the `always` trigger for unconditional charge-scaled augmentations — Dropkick t2, the single-foe condition-grant family reading the ability's own mutations — Valiant t2, Provoke t1, Showdown t2, and the post-application `slay`/`collide` triggers decided by the same reactive dry run that derives the ability's own clauses — Umbra t1, Valiant t1, Dragon Dive t1), three program-level talents (Demon Cutter t2's pre-ability rush, charge: rush 3; Draken Cross t2's charged medium blasts; and Pyre t1's comeback ally immunity) are implemented in the ability programs and gated on the equipped choice through the projected `talents` surface, and the 256 documented rows carry their kernel need; the actor's durable `talents` map is validated by the room validator and protocol schema — see `rules-foundations.md` §8. The unfinished Job scope is the 42 documented traits, the 256 documented talents, masteries, Limit Breaks, summon action suites, and documented table-facing choices—not a source-only set of Job abilities. |
