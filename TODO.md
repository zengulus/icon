...
> Heartfire/Aethershard/Implode/Pyroclast/Blackstar), Chanter (all 14
...
> was untouched, and the full U8×U16 adversarial matrix still passes), and
> Knave (all 13 source reads + 4 attack-target reads in Low Blow/The Hook/
> Provoke/Revenge/Indignation/Riposte/Dire Parry/Dark Knight/Strongarm/
> Intimidate/Sucker Punch/Bleak Mercy/Sweet Torment — plannedRush's
...
- **U7 tranche 21 — teleport missing-mover fail-closed repair; U7
  completion audit; U7 becomes AUTHORITATIVE; next = U5-core (2026-09-02)
  — DONE.** (a) VERDICT ON TRANCHES 19-20: verified, with ONE real
  correctness repair. All claims held: five-family U7 residual
  classification; teleport range genuinely was measured from a degenerate
  size-1 POINT (the legality call carried no `originSize`, so the
  operator's point baseline applied regardless of the mover's footprint);
  p.92 measures from the edge of the mover's actual footprint; the new
  Size-2 edge behavior is correct and Size-1 unchanged; the aura /
  creationSpatial / RuleArea.origin / rebound boundaries are genuine
  specialist carriers (eligibility scan / record-carried CAPTURED /
  inert / unwired — none reinterprets LIVE-vs-CAPTURED or maps a selector
  to a frame); the no-`ResolvedReferenceFor<R>` redesign decision is
  sound; zero source promotion. (b) THE REPAIR: `originSize: mover?.size
  ?? 1` was a REAL fail-open — `size` is REQUIRED on RuleActorView /
  EncounterActor, so the fallback fires exactly when the mover record is
  MISSING, silently restoring the point frame and accepting a teleport
  command for a nonexistent mover. `chosenTeleportDestination` now
  resolves the mover record fail-closed (`selector.actor-missing` thrown
  BEFORE any destination legality is accepted; never Size 1 from absence);
  the `u7-teleport-footprint-origin` guard now REJECTS an optional-
  chained/`?? 1` size read (non-optional `.size` only), mutation-tested
  both ways; adversarial fixture proves a missing mover with a fully legal
  destination fails closed while Size-1/Size-2 valid behavior is unchanged.
  (c) U7 COMPLETION DECISION: AUTHORITATIVE. Fresh audit of every
  remaining origin/frame consumer — (A) resolves through the U7
  vocabulary: candidate `rangeOrigin`, choice position legality,
  evaluate-value distance endpoints, evaluate-query LoS/entity reads;
  (B) carries an already-resolved frame with a written boundary: teleport,
  creationSpatial (record-carried CAPTURED; reducer consumes as recorded),
  aura (LIVE carrier-scan), foe/attack/range measurement (factual live
  records); (C) inert/non-spatial: RuleArea.origin, rebound, runtime.ts
  actorId (U1 identity for cost). No duplicate origin selection, footprint
  sizing, point-range, entity geometry, captured-vs-live re-derivation,
  implicit actorId fallback, or Size-1 fallback for a multi-space actor
  remains in a generic measurement path. The structural gate "every
  declared origin kind on the typed vocabulary" was broader than the
  ontology's ownership model (the plan's own Consumers-to-migrate rows +
  t8c/t8d proofs classified the carriers as disjoint Q); clarified, not
  redefined: U7 owns vocabulary + resolution + canonical metric; carrier
  schemas store already-resolved frames and are NOT migrated. (d) NEXT
  DEPENDENCY-DRIVEN TRANCHE: **U5-core** — with U7 authoritative, U3's
  gate (U1✓+U2✓+U5-core+U7✓) leaves U5-core as the only blocker; U9
  (U1+U2) is dependency-complete but the canonical T2/T3/T4 phase order
  (T2 U7/U3/U5/U6-core/U4 before T4 U9/U10) selects the T2 continuation
  — the same phase-order tiebreak tranche 19 used.  EVIDENCE: 2177 tests
  (+2: adversarial missing-mover fixture + guard-restoration mutation
  test); architecture audit clean with the tightened guard; automation /
  source-fidelity strict / build clean (source-fidelity.md regenerated by
  its owner to register `claim:foundations:u7-authoritative`); blocker
...
- **U1 content-authoring tranche 6 — Knave pure LIVE-slot reads
  ...
  routes every pure live-slot reference through the adapter: source reads →
  `resolveSourceActor` in Low Blow, The Hook, Provoke, Revenge, Indignation,
  Riposte, Dire Parry, Dark Knight, Strongarm, Intimidate, Sucker Punch,
  Bleak Mercy, Sweet Torment (13); primary attack-target reads →
  `resolveAttackTarget` in Low Blow, The Hook, Indignation, Bleak Mercy (4)
  — identical LIVE/absent-singular semantics with the resolvers' gates
  untouched. The six retained CAPTURED sites stay caller-owned: `plannedRush`
  (its `actorId` is a helper PARAMETER, not the source slot — no arbitrary-id
  adapter op invented), Dire Parry (`triggerSourceId ??
  input.actorIds?.target?.[0]` precedence), Strongarm (`input.actorIds`
  target + `passedId` loop deref), Intimidate and Sucker Punch
  (`input.actorIds` targets). Dark Knight's nearest-foe read is U3 query + U4
  player-choice (p.143 equidistant-choice) and was NOT touched: equidistant
  foes still fail closed with no id tie-break or iteration-order resolution.
  Guard pins Knave via contentAdapterSurface; mutation test covers a Knave
  revert (exactly one routing violation) and proves the retained
  captured/precedence dereferences alone do not trigger the pin. Evidence:
  +5 fail-closed/U4-parity tests in `reference-authoring.test.ts` (Low Blow
  ghost attackTargetId → `reference.missing-actor`; Bleak Mercy
  absent-singleton no-op; Dark Knight equidistant →
  `choice.direction-ambiguous`; Dire Parry trigger-source precedence;
  Strongarm recorded-input target), +1 engine-level insertion-order test in
  `knave.test.ts`, the full Knave suite (23) green through the engine path
  (valid-state semantics preserved; malformed refs fail closed; replay
  byte-identical). The tranche removed exactly 17 PURE_LIVE_REFERENCE sites
  (Knave 17 → 0), moving the machine-derived residual from
  192 = 137 + 54 + 1 to 175 = 120 + 54 + 1 (per `npm run audit:u1-residual`;
  no hand-maintained totals). 0 direct dereferences; U1 remains PARTIAL.
  Full suite 2000 green, audits green, census byte-stable at 427, zero
  source promotion.
