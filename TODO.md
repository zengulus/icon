...
| F3 | Terrain-create / entity-create recipe primitives | **entity-create reconciled (2026-08-29, F3 + corrective pass)**: the authority already exists (`kernels/entity-creation.ts` `validateEntityCreation` + `entityMutation` + `freeCellsInRange`), so the 16 `{entity-create}` singletons were coarse keyword residues, reclassified to precise residual blockers. The shared `summonEntity` seam now declares INTENT — a single creation mutation with an ordered candidate list + requested `count` + a paired `creationSpatial {origin, originSize, maxRange}` — and the reducer's `validateEntityCreation` is the single legality/selection authority (picks the first `count` legal candidates; emits distinct per-cell ids for a `count>1` creation, fixing a generatedId collision). Adopted by warden/seer/geomancer ordinary summons; the remaining `freeCellsInRange`+`entityMutation` sites are tracked for migration or are deliberate cell-dependent exceptions. `entity-create` is retired as a blocker label — **0 unresolved occurrences, absent from the greedy simulation**. 0 promoted (honest). **Next census-selected family (regenerated 2026-08-29): `{fly-grant}` (17 immediate)**; `terrain-create` (16 immediate) is now second | 16 reclassified; 0 promoted | Census `{entity-create}` cleared; next = `{fly-grant}` (17 immediate) |
