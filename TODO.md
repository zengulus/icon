...
| F3 | Terrain-create / entity-create recipe primitives | **entity-create CLOSED as a family (2026-08-29)**: the authority already exists (`kernels/entity-creation.ts` + `entityMutation` + `freeCellsInRange`), so the 16 `{entity-create}` singletons were NOT missing a primitive — they were coarse keyword residues and were reclassified to precise residual blockers (0 `{entity-create}` immediate left; compound-only remains). A shared `summonEntity` seam landed and is the single summon seam for ability resolvers. 0 promoted (honest — no unit's full blocker set emptied). **terrain-create is the next census-selected family**: 16 immediate (54 total) — generalize program-local terrain/object creation into parameterized factories under the same single authority | 16 reclassified; 0 promoted | Census `{entity-create}` immediate cleared; next = `{terrain-create}` (16 immediate) |
