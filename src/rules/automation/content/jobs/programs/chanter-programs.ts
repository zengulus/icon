...
const holyEffects: RuleResolver = (context) => {
  ...
  if (target.position) {
    ...
    // (Recover; the p.103/p.172 Cure primers) define only the effect ("a
    // character that's cured gains 4 vigor, or a vigor surge if they're
    // bloodied... save against all statuses"). Esper III (p.249 "Cures can
    // target foes and deal fray damage to them instead of any of its other
    // effects") is the FOE MODE on an already-legal category — its operative
    // clause replaces the normal cure effect with fray damage; the book
    // writes "can target X" both to grant outside-keyword categories (p.92
    // summons "can only be targeted if an ability can target all characters";
    // Mercy I "Your cures can target defeated characters" — the defeated are
    // in NO p.92 keyword, unlike foes) and to declare category legality for a
    // new effect. Erenbrass Aspected (p.249) extends an "ally"-worded effect
    // (lexically foe-exclusive) and says nothing about "character"-worded
    // ones. The engine's Esper III mode is not wired (relics are
    // documented-unresolved), so a foe recipient gets the standard cure
    // effect today — the relic's fray-damage replacement is the exact
    // missing capability to wire when relics land. The WHICH character is a
    // mandatory choose-one: recorded under `holy-cure` and validated as a
    // member of the eligible set. The attacked foe itself is always an
    // eligible living character at distance 0, so the eligible set is never
    // empty in a valid use — a missing recording is a missing REQUIRED
    // decision (fail closed `choice.actor-required`), never a vacuous pass;
    // a recorded non-member fails closed (`choice.actor-ineligible`);
    // several recordings fail (`choice.actor-count`). Defeated/off-board
    // characters are excluded by the U3 base eligibility (Mercy I's
    // defeated grant is the explicit extension).
