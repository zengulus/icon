...
export type RuleMutation =
  ...
  | { kind: 'entity'; sourceId: string; operation: 'summon' | 'create' | 'remove'; entityType: string; ownerId: string; positions: Position[]; count: number; state: Readonly<Record<string, string | number | boolean | null>>; duration?: RuleDuration; /** Replay-safe creation spatial contract, computed at command time: the resolved origin position, the origin actor's size for the p.92 footprint-distance metric, and an optional maximum range. A PAIRED invariant: the contract always carries a valid origin — the reducer rejects a range without a valid origin defensively even if a malformed mutation bypasses the type. */ creationSpatial?: { origin: Position; originSize: number; maxRange?: number } }
