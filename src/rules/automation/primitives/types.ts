...
export type RuleEffect =
  ...
  | { kind: 'entity'; operation: 'summon' | 'create' | 'remove'; entityType: string; owner: RuleSelector; positionInput?: string; count?: RuleNumber; state?: Readonly<Record<string, string | number | boolean | null>>; duration?: RuleDuration; /** Source-declared creation origin for LoS/range validation. Evaluated at command time against the runtime state (e.g. the owner's position). */ origin?: RuleSelector; /** The origin actor's size (RuleNumber) for p.92 footprint-distance range validation. */ originSize?: RuleNumber; /** Maximum footprint distance from the evaluated origin. Only enforced when `origin` is provided. */ maxRange?: number }
...
export type RuleMutation =
  ...
  | { kind: 'entity'; sourceId: string; operation: 'summon' | 'create' | 'remove'; entityType: string; ownerId: string; positions: Position[]; count: number; state: Readonly<Record<string, string | number | boolean | null>>; duration?: RuleDuration; /** Replay-safe creation origin for reducer-level LoS/range enforcement. Computed at command time; the reducer validates against authoritative state on replay. */ creationOrigin?: Position; /** The origin actor's size for p.92 footprint-distance range validation. */ creationOriginSize?: number; /** Maximum footprint distance from creationOrigin. Only enforced when creationOrigin is present. */ creationMaxRange?: number }
