...
export function CharacterEditor() {
  ...
  function addRelic(relicId: string) {
    ...
    update({ relics: [...draft.relics, { relicId, rank: 1, dustInfused: 0 }] });
  }
      ...
      <div className="editor-layout">
        <div className="editor-main">
          ...
          <section className="sheet-section">
            ...
            <div className="relic-manager"><div className="subheading"><div><h3>Relics</h3><p>Relic slots unlock at levels 2, 6, and 9.</p></div><select value="" onChange={(event) => addRelic(event.target.value)}><option value="">Add relic…</option>{RELICS.filter((relic) => !draft.relics.some((selected) => selected.relicId === relic.id)).map((relic) => <option value={relic.id} key={relic.id}>{relic.name}</option>)}</select></div>{draft.relics.map((selected) => { const relic = RELICS.find((item) => item.id === selected.relicId)!; return <article key={selected.relicId}><div><strong>{relic.name}</strong><small>{relic.description}</small></div><label>Rank<select value={selected.rank} onChange={(event) => update({ relics: draft.relics.map((item) => item.relicId === selected.relicId ? { ...item, rank: Number(event.target.value) as 1 | 2 | 3 | 4 } : item) })}><option value="1">I</option><option value="2">II</option><option value="3">III</option><option value="4">Aspected</option></select></label><label>Infused dust<input type="number" min="0" value={selected.dustInfused} onChange={(event) => update({ relics: draft.relics.map((item) => item.relicId === selected.relicId ? { ...item, dustInfused: Number(event.target.value) } : item) })} /></label><button className="text-button danger" onClick={() => update({ relics: draft.relics.filter((item) => item.relicId !== selected.relicId) })}>Remove</button></article>; })}</div>
          </section>
